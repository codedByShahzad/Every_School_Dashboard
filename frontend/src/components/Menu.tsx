      ...
      {menuItems.map((item, index) => (
        <div key={index} className="flex flex-col gap-2 ">
          ...
          {item.items.map((value, id) => {
            if (value.visible.includes(role)) {
              return (
                <Link
                  href={value.href}
                  key={id}
                  className="flex items-center justify-center lg:justify-start gap-4 text-gray-500 py-2 hover:bg-skyLight md:px-2 rounded-md"
                >
